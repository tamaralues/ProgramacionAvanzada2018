...
def leer_ciudades(ruta_archivo_Ciudad):
    with open(ruta_archivo_Ciudad, "r", encoding="utf-8") as linea:
        limpiador = lambda x: x.strip().split(",")
        for p in linea:
            City= Ciudad(*limpiador(p))
            yield City



def leer_paises(ruta_archivo_paises):
    with open(ruta_archivo_paise, "r", encoding="utf-8") as linea:
        limpiador = lambda x: x.strip().split(",")
        for p in linea:
            Country= Pais(*limpiador(p))
            yield Country

def leer_personas(ruta_archivo_personas):
    with open(ruta_archivo_personas, "r", encoding="utf-8") as linea:
        limpiador = lambda x: x.strip().split(",")
        for p in linea:
            Per= Persona(*limpiador(p))
            yield Per
